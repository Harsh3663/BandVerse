...
import { portfolioMediaCreateSchema } from "@/backend/shared/validation/schemas";
import { z } from "zod";
...
export async function GET(request: Request, context: RouteContext) {
  ...
  if (!parsed.ok) {
    finish(400, "/api/v1/portfolio/:performerId/media");
    return jsonError(parsed.error, requestId);
  }
  const items = await container.portfolio.listMedia(parsed.value.performerId);
  finish(200, "/api/v1/portfolio/:performerId/media");
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!auth.ok) {
    finish(auth.error.status, "/api/v1/portfolio/:performerId/media");
  ...
  const parsed = parseWithSchema(portfolioMediaCreateSchema, body.value);
  if (!parsed.ok) {
    finish(400, "/api/v1/portfolio/:performerId/media");
    return jsonError(parsed.error, requestId);
  }
  ...
  try {
    const item = await container.portfolio.createMedia({
      performerId: parsedParams.value.performerId,
      ...parsed.value,
    });
    finish(201, "/api/v1/portfolio/:performerId/media");
    return jsonOk(item, { status: 201, requestId });
  } catch (error) {
    const appError = toPortfolioError(error);
    finish(appError.status, "/api/v1/portfolio/:performerId/media");
