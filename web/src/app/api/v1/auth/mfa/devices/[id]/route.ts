...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  if (!auth.ok) {
    finish(401, "/api/v1/auth/mfa/devices/:id");
  ...
  if (!parsed.ok) {
    finish(400, "/api/v1/auth/mfa/devices/:id");
    return jsonError(parsed.error, requestId);
  }

  const revoked = await container.mfa.revokeTrustedDevice(
    auth.value.userId,
    parsed.value.id,
  );
  if (!revoked) {
    finish(404, "/api/v1/auth/mfa/devices/:id");
    return jsonError(notFoundError("Trusted device", parsed.value.id), requestId);
  }

  finish(200, "/api/v1/auth/mfa/devices/:id");
