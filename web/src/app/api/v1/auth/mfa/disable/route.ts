...
export async function POST(request: Request) {
  ...
  if (!auth.ok) {
    finish(401, "/api/v1/auth/mfa/disable");
    return jsonError(auth.error, requestId);
  }

  const body = await parseJsonBody(request);
  if (!body.ok) {
    finish(400, "/api/v1/auth/mfa/disable");
  ...
  try {
    const status = await container.mfa.disable(auth.value.userId, parsed.value.code);
    finish(200, "/api/v1/auth/mfa/disable");
